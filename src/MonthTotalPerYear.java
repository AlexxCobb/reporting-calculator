...
public class MonthTotalPerYear {
    ...
    boolean checkReports() {
        ...
        HashMap<String, ArrayList<MonthTransaction>> keys = monthlyReport.monthRep;

        for (String month : keys.keySet()) {
            HashMap<Integer, Integer> expensesToProfit = new HashMap<>();
            ArrayList<MonthTransaction> reportMonth = monthlyReport.monthRep.get(month);
            int sumExp = 0;
            int sumProfit = 0;
            for (MonthTransaction transaction : reportMonth) {
